...
std::map<std::string, std::vector<ScoreMovePair>> loadOpeningsFromString(const std::string& input) {
    std::map<std::string, std::vector<ScoreMovePair>> fenToBestMove;
    std::istringstream inputStream(input); // Use a string stream instead of file stream
    std::string line;

    while (getline(inputStream, line)) { // Read from string stream
        auto delimiterPos = line.find(" | Score: ");
        std::string movesStr = line.substr(0, delimiterPos);
        int score = std::stoi(line.substr(delimiterPos + 10)); // "+ 10" to skip past " | Score: "
    ...
    // Sorting each vector of moves by score after all moves are inserted
    for (auto& [fen, pairs] : fenToBestMove) {
        std::sort(pairs.begin(), pairs.end()); // Make sure ScoreMovePair has an operator< defined for sorting
    }
